package lt.eif.viko.mjakovcenko.banksoap.banksoap.servlet;
...
public class TomcatServlet extends HttpServlet {
    ...
    private void servePdfFile(HttpServletResponse response) throws IOException {
        Resource resource = new ClassPathResource("static/Client_1.pdf");
        if (!resource.exists()) {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        if (request.getRequestURI().contains("/static/Client_1.pdf")) {
            ...
            out.println("<p><a href='/pdf/static/Client_1.pdf'>Open PDF File</a></p>");
            out.println("</div>");
            out.println("</body>");
            out.println("</html>");
        }
